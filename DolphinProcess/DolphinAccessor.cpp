...
namespace DolphinComm
{
  ...
  std::string DolphinAccessor::getFormattedValueFromPointer(const u32 ramIndex, Common::MemType memType,
                                                            size_t memSize, Common::MemBase memBase,
                                                            bool memIsUnsigned)
  {
    char buffer[sizeof(u32)];
    copyRawMemoryFromCache(buffer, ramIndex, sizeof(u32));
    std::cout << std::hex << (0xFF & buffer[0]) << (0xFF & buffer[1]) << std::endl;
    return Common::formatMemoryToString(&m_updatedRAMCache[ramIndex], memType, memSize, memBase,
                                        memIsUnsigned, Common::shouldBeBSwappedForType(memType));
  }
